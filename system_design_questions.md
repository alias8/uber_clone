...
- **A real scaling concern here:** the surge calculation's "pending nearby rides" count currently queries Postgres directly with a lat/lng bounding-box filter, synchronously inside the `POST /rides` request path. There already is a composite index for it (`idx_rides_status_location ON rides (status, pickup_lat, pickup_lng)`, `V2__ride_indexes.sql`) — and it's a reasonably good fit, since `status = 'REQUESTED'` as the leading column is highly selective (most rides move out of `REQUESTED` within seconds to minutes, so the index scan stays small regardless of total historical ride volume). The remaining issue isn't the index, it's that this is a **synchronous Postgres round-trip inside the fast request path at all** — real latency even when well-indexed, for data (`REQUESTED` rides) that's already being tracked in Redis for dispatch purposes anyway. A live per-grid-cell counter in Redis, incremented/decremented as rides enter/leave `REQUESTED`, would remove the DB round-trip from this hot path entirely, consistent with how the rest of the dispatch path already avoids Postgres.
